...
class MaskDINODecoder(nn.Module):
    ...
    def forward(self, x, mask_features, masks, targets=None):
        ...
        if self.two_stage:
            ...
            outputs_class, outputs_mask, decoder_output = self.forward_prediction_heads(tgt_undetach.transpose(0, 1), mask_features)
            ...
            interm_outputs['decoder_output'] = decoder_output
        ...
        if self.initial_pred:
            outputs_class, outputs_mask, decoder_output = self.forward_prediction_heads(tgt.transpose(0, 1), mask_features, self.training)
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
            predictions_decoder_output.append(decoder_output)
        ...
        for i, output in enumerate(hs):
            outputs_class, outputs_mask, decoder_output = self.forward_prediction_heads(output.transpose(0, 1), mask_features, self.training or (i == len(hs)-1))
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
            predictions_decoder_output.append(decoder_output)
        ...
        decoder_output = predictions_decoder_output[-1]
        if mask_dict is not None:
            decoder_output = decoder_output[:, mask_dict['pad_size']:, :]
    ...
    def forward_prediction_heads(self, output, mask_features, pred_mask=True):
        ...
        return outputs_class, outputs_mask, decoder_output
