...
class MaskDINO(nn.Module):
    ...
    def forward(self, batched_inputs, targets=None, compute_losses=False):
        """
        Args:
        ...
        if self.training:
            ...
            if "instances" in batched_inputs[0]:
                ...
                outputs, transformer_encoder_features, mask_features = self.sem_seg_head(features,targets=targets)
            else:
                outputs, transformer_encoder_features, mask_features = self.sem_seg_head(features)
            outputs, mask_dict = outputs
            # bipartite matching-based loss
            #losses = self.criterion(outputs, targets, mask_dict)
            outputs["image_features"] = features["res5"]
            outputs["embedded_memory_features"] = transformer_encoder_features
            outputs["maskdino_mask_features"] = mask_features
            outputs["mask_dict"] = mask_dict
    ...
    def prepare_targets_detr(self, targets, images):
        ...
        for targets_per_image in targets:
            # pad gt
            h, w = targets_per_image.image_size
            image_size_xyxy = torch.as_tensor([w, h, w, h], dtype=torch.float, device=self.device_sem_seg_head)
            ...
            new_targets.append(
                {
                    ...
                    "masks": padded_masks,
                    "boxes": box_ops.box_xyxy_to_cxcywh(targets_per_image.gt_boxes) / image_size_xyxy
                }
            )
        return new_targets
