...
class ContrastiveMultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def forward(self, x, mask_features, tasks, mask = None):
        ...
        outputs_class, outputs_mask, attn_mask, decoder_output = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[0], i=0)
        ...
        for i in range(self.num_layers):
            ...
            outputs_class, outputs_mask, attn_mask, decoder_output = self.forward_prediction_heads(output, mask_features, attn_mask_target_size=size_list[(i + 1) % self.num_feature_levels], i=i+1)
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
            predictions_decoder_output.append(decoder_output)
