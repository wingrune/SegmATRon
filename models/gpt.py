...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pos_len = config.NUM_QUERIES * config.NUM_ACTIONS + config.NUM_ACTIONS
        self.pos_emb = nn.Parameter(torch.zeros(1, self.pos_len, config.EMBEDDING_DIM))
        # self.seq_pos_embed = nn.Parameter(torch.zeros(1, self.block_size, config.EMBEDDING_DIM), requires_grad=False)
        
        self.embed_dim = config.EMBEDDING_DIM
        self.img_len = config.INPUT_RESOLUTION // 32 * config.INPUT_RESOLUTION // 32
        self.block_size = config.BLOCK_SIZE
        self.seq_pos_embed = nn.Parameter(torch.zeros(1, self.block_size, config.EMBEDDING_DIM), requires_grad=True)
    ...
    def init_pos_emb(self):
        ...
        seq_sin_embed = get_1d_sincos_pos_embed(self.embed_dim // 2, 2*self.num_actions + 1)
        seq_pos_embed = torch.zeros((1, 2*self.num_actions + 1, self.embed_dim))
        seq_pos_embed[:, :, self.embed_dim // 2:] = torch.from_numpy(seq_sin_embed).float()

        pred_sin_embed = get_1d_sincos_pos_embed(self.embed_dim // 2, self.num_queries)
        pred_pos_embed = torch.zeros((1, self.num_queries, self.embed_dim))
        pred_pos_embed[:, :, self.embed_dim // 2:] = torch.from_numpy(pred_sin_embed).float() + 0.2

        action_sin_embed = get_1d_sincos_pos_embed(self.embed_dim // 2, self.num_actions)
        action_pos_embed = torch.zeros((1, self.num_actions, self.embed_dim))
        action_pos_embed[:, :, :self.embed_dim // 2] = torch.from_numpy(action_sin_embed).float()

        pos_emb = torch.zeros((1, self.block_size, self.embed_dim))
        for i in range(self.num_actions):
            pos_emb[:,(self.img_len+self.num_queries)*i:(self.img_len+self.num_queries)*i+self.img_len] = img_pos_embed + seq_pos_embed[:,i*2,:]
            pos_emb[:,(self.img_len+self.num_queries)*i+self.img_len:(self.img_len+self.num_queries)*(i+1)] = pred_pos_embed + seq_pos_embed[
                                                                                                   :,i*2+1,:]
        pos_emb[:, (self.block_size - self.num_actions):, :] = action_pos_embed[:, :, :] + seq_pos_embed[:, -1, :]
